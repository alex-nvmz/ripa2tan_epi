...
We are missing the population for 2022, and this is vital for the analysis. We will impute it.

We will impute each univariate time series when appropriate by **Kalman smoothing**. For the yearly data (population),  a state space model with local trend will be used. For the monthly data, a state space model with local trend and yearly seasonal component will be used.
...
Population has been increasing linearly each year. We will impute the population of 2022 with Kalman smoothing, assuming a local trend model.
...
# Prepare df with missings for 2022
pop_impute <- pop_impute |> 
  full_join(data.frame(date = seq(total_range[1], total_range[2], by="year"))) |> 
  complete(district, date) |> 
  drop_na(district)

# Prepare df with Moshi time series
pop_impute_moshi <- pop_impute |> 
  filter(district == "Moshi") |> 
  arrange(date)

# Create ts object for imputation
pop_impute_moshi_ts <- ts(pop_impute_moshi$population, frequency=1)

# Impute
pop_impute_moshi <- pop_impute_moshi |> 
  mutate(
    population_imp = na_kalman(pop_impute_moshi_ts, model="StructTS", smooth=TRUE, type="trend")
  )

# Repeat for Siha
pop_impute_siha <- pop_impute |> 
  filter(district == "Siha") |> 
  arrange(date)

pop_impute_siha_ts <- ts(pop_impute_siha$population, frequency=1)

pop_impute_siha <- pop_impute_siha |> 
  mutate(
    population_imp = na_kalman(pop_impute_siha_ts, model="StructTS", smooth=TRUE, type="trend")
  )

# Merge
pop_impute <- pop_impute_moshi |> 
  full_join(pop_impute_siha)
...
Temperature, rain and greenness are missing for 2022, and this data is not available. We could make forecasts for this data, but I think including the forecasts in the regression could bias our results. We should exclude 2022 from the regression.

Rain is missing for Siha for 2021. I think we should forecast it.

A multivariate state space model with the rain variables for Moshi and Siha could be useful for the imputation (i.e. consider the multivariate normal vector [rain_siha, rain_moshi]), but we will do the forecasts in the univariate case for simplicity when implementing the code.
